...
```python
# v2.2.8: price=0 is now rejected for Limit and Stop-Loss-Limit orders
...
```python
# Before (v2.2.8, "quick-modify" path)
client.modify_order(
...
`on_error`, then call `subscribe`). v2.2.8 uses a modern **async/await** client
