...
from neo_api_client.websocket.HSWebSocketLib import HSIWebSocket, HSWebSocket
...
class NeoWebSocket:
    ...
    def start_websocket(self):
        ...
        self.hsWebsocket.open_connection(
            WEBSOCKET_URL,
            self.access_token,
            self.sid,
            self.on_hsm_open,
        ...
        if message:
            if isinstance(message, str):
                ...
                if req_type == "unsub":
                    ...
                    if len(self.un_sub_channel_token) == 0 and self.token_limit_reached:
                        self.sub_list = []
                        self.channel_tokens = {}
                        self.un_sub_channel_token = {}
                    if self.on_message:
                ...
                if (
                    len(self.sub_list) >= 1
                    and self.is_message_for_subscription(message)
                    and self.on_message
                ):
    ...
    def is_message_for_subscription(self, message):
        ...
        is_for_sub = False
        keys_in_sublist = list(
            {outer_key for data_dict in self.sub_list for outer_key in data_dict}
        )
        # print("sublist keys ",keys_in_sublist)
        for item in message:
            if "tk" in item and item["tk"] in keys_in_sublist:
                is_for_sub = True
    ...
    def remove_items(self, un_sub_json):
        for unsubscribe_token in un_sub_json:
            ...
            sub_type_value = unsubscribe_token[list(unsubscribe_token.keys())[0]][
                "subscription_type"
            ]
            ...
            for channel_token_list in self.channel_tokens.values():
                for channel_token_dict in channel_token_list:
                    for _channel_token_key, channel_token_value in channel_token_dict.items():
                        if (
                            token_value == channel_token_value["instrument_token"]
                            and segment_value == channel_token_value["exchange_segment"]
                            and sub_type_value == channel_token_value["subscription_type"]
                        ):
    ...
    def get_formatted_data(self, instrument_tokens):
        ...
        for item in instrument_tokens:
            for k, v in item.items():
                if isinstance(v, dict) and "exchange_segment" in v and "instrument_token" in v:
                    if scrips != "":
                        scrips += "&"
    ...
    def format_tokens_live(self, instrument_tokens):
        scrips = ""
        if (
            isinstance(instrument_tokens, dict)
            and "exchange_segment" in instrument_tokens
            and "instrument_token" in instrument_tokens
        ):
            if scrips != "":
                scrips += "&"
            scrips += (
                instrument_tokens["exchange_segment"]
                + "|"
                + str(instrument_tokens["instrument_token"])
            )
        return scrips
    ...
    def format_un_sub_list(self, instrument_tokens):
        ...
        for instrument_token in instrument_tokens:
            if (
                isinstance(instrument_token, dict)
                and "exchange_segment" in instrument_token
                and "instrument_token" in instrument_token
            ):
                if scrips != "":
                    scrips += "&"
                scrips += (
                    ...
                    + "|"
                    + str(instrument_token["instrument_token"])
                )
        return scrips
    ...
    def call_quotes(self):
        ...
        if self.quotes_index:
            ...
            if quote_type and quote_type.strip().lower() == "market_depth":
    ...
    def quote_type_validation(self, quote_type):
        Q_type = True
        if quote_type and str(quote_type).strip().lower() not in [
            "market_depth",
            "ohlc",
            "ltp",
            "52w",
        ...
        for channel, token_list in channel_tokens.items():
            for tokens in token_list:
                ...
                req_params1 = json.dumps(
                    {
                        "type": tokens[0]["subscription_type"],
                        "scrips": scrips,
                        "channelnum": channel,
                    }
                )
    ...
    def quote_type_filter(self, new_dict, quote_type):
        if quote_type:
            resp_dict = {
                "instrument_token": new_dict["instrument_token"],
    ...
    def quote_response_formatter(self, message):
        ...
        for item in message:
            if "tk" in item and item["tk"] in quotes_arr_list:
    ...
    def un_subscription(self):
        for channels, token_list in self.un_sub_channel_token.items():
            ...
            req_params1 = json.dumps({"type": sub_type, "scrips": scrips, "channelnum": channel})
            self.hsWebsocket.hs_send(req_params1)
    ...
    def un_subscribe_list(self, instrument_tokens, isIndex=False, isDepth=False):
        ...
        if self.input_validation(instrument_tokens):
            extracted_tokens = [
                {
                    "instrument_token": item[key]["instrument_token"],
            ...
            if self.hsWebsocket and self.is_hsw_open == 1:
                ...
                print(
                    "Socket Connection has been closed, So! The scripts are already un-subscribed!"
                )
